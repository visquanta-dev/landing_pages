 ...
 */
import { isForexTradingSignalsBusiness } from './site-niche'
...
export function generateSmsTemplates(legalEntityName: string, dbaName: string, phoneHelp?: string, email?: string, businessType?: string) {
  ...
  const isForexTradingSignals = isForexTradingSignalsBusiness(normalizedType)
  const messageTypes = isCcw
    ...
    : isForexTradingSignals
    ? 'forex signal alerts, market analysis updates, account notifications, appointment confirmations, reminders, support follow-ups, and account-related service notifications'
