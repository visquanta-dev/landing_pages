...
import { usableImageUrl } from '@/lib/image-url'
...
export default function LandingPage({ dealer: d }: { dealer: Dealership }) {
  ...
  function handleDateChange(e: React.ChangeEvent<HTMLInputElement>) {
    if (isGym || isInsurance || isCustomService) return
  ...
  const smsConsentText =
    d.sms_consent_text ||
    (isGym
      ? 'By checking this optional SMS consent box and submitting this form, you agree to receive recurring customer care text messages from this gym, including appointment confirmations, class booking confirmations, reminders, rescheduling updates, missed appointment follow-ups, and account-related service notifications. Message frequency varies based on your requests and appointments. Message and data rates may apply. Reply HELP for help. Reply STOP to unsubscribe. Consent is not a condition of purchase. We will not share your SMS opt-in or consent status with third parties for purposes unrelated to providing these messaging services.'
      : isInsurance
    ...
    <>
      ...
      <nav id="lp-nav">
        <a href="#" onClick={(e) => { e.preventDefault(); showPage('main') }} style={{ display: 'flex', alignItems: 'center', gap: 14, textDecoration: 'none' }}>
          {logoUrl && <img src={logoUrl} alt={d.dealership_name} style={{ height: 40, objectFit: 'contain' }} onError={(e) => e.currentTarget.remove()} />}
        ...
        <div className="lp-nav-links" style={{ display: 'flex', gap: 36, alignItems: 'center' }}>
          {['Home', 'How It Works', isCustomService ? 'Services' : isGym ? 'Classes & Training' : isInsurance ? 'Coverage' : 'Vehicles', 'Contact'].map((label) => {
            const id = label === 'Home' ? '' : label === 'How It Works' ? 'how' : label === 'Contact' ? 'info' : (isCustomService || isGym ? 'services' : isInsurance ? 'products' : 'vehicles')
            return (
              <a key={label} href={`#${id}`}
                onClick={(e) => {
                  e.preventDefault()
        ...
        <div>
          ...
          <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', inset: 0 }}>
              {heroBg && <img src={heroBg} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', filter: isDryCleaner ? 'brightness(0.42) saturate(0.85)' : 'brightness(0.3) saturate(0.7)' }} onError={(e) => e.currentTarget.remove()} />}
            ...
            <div className="lp-hero-grid" style={{ position: 'relative', zIndex: 1, maxWidth: 1280, margin: '0 auto', width: '100%', padding: '140px 48px 100px', display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: 80, alignItems: 'center' }}>
              ...
              <div className="lp-hero-visual anim-up anim-d3" style={{ position: 'relative' }}>
                {heroCard && (
                  <div data-hero-card="true" style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 40px 100px rgba(0,0,0,0.5)' }}>
                    <img src={heroCard} alt="" style={{ width: '100%', height: 380, objectFit: 'cover', display: 'block' }} onError={(e) => e.currentTarget.closest('[data-hero-card]')?.remove()} />
                    ...
                    <div style={{ position: 'absolute', bottom: 20, left: 20, right: 20, background: 'rgba(9,9,9,0.85)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
                      {logoUrl && <img src={logoUrl} alt="" style={{ width: 44, height: 44, objectFit: 'contain', background: '#fff', borderRadius: 8, padding: 5 }} onError={(e) => e.currentTarget.remove()} />}
                      <div>
            ...
            <div style={{ maxWidth: 1000, margin: '0 auto' }}>
              ...
              <div className="lp-3col" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
                {(isCcw ? [
                  ...
                  { num: '03', title: 'We Confirm Your Slot', desc: `We work directly with ${d.dealership_name} to confirm your appointment, booking, or consultation via SMS.` },
                ] : isGym ? [
                ...
                <div className="lp-3col" style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(customServices.length, 3)}, 1fr)`, gap: 24 }}>
                  {customServices.map((s, i) => (
                    <div key={i} className={`lp-reveal ${i > 0 ? `lp-d${i}` : ''}`}
                      ...
                      {s.image ? (
                        <img src={s.image} alt="" style={{ width: '100%', height: 180, objectFit: 'cover', display: 'block' }} onError={(e) => e.currentTarget.remove()} />
                      ) : (
                  ...
                  {vehicles.map((v, i) => (
                    <div key={i} className={`lp-reveal ${i > 0 ? `lp-d${i}` : ''}`}
                      ...
                      <div style={{ height: 240, background: 'linear-gradient(145deg, #0d0d0d, #1a1a1a)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                        <img src={v.image_url} alt={v.name} style={{ width: '88%', maxHeight: 200, objectFit: 'contain', filter: 'drop-shadow(0 16px 40px rgba(0,0,0,0.6))' }} onError={(e) => e.currentTarget.remove()} />
                      </div>
          ...
          <section id="booking" className="lp-section" style={{ padding: '120px 48px', position: 'relative' }}>
            <div style={{ maxWidth: 680, margin: '0 auto' }}>
              ...
              <h2 className="fd lp-reveal lp-d1" style={{ fontSize: 'clamp(36px, 5vw, 56px)', fontWeight: 500, letterSpacing: '-0.02em', marginBottom: 16, lineHeight: 1.1, color: '#FFFFFF' }}>{bookingHeading}</h2>
                ...
                <div style={{ marginTop: 28, padding: 24, border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, background: 'rgba(255,255,255,0.015)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                    <input
                      ...
                      style={{ marginTop: 3, width: 18, height: 18, accentColor: c, flexShrink: 0, cursor: 'pointer' }}
                    />
                    <label htmlFor="sms-consent-checkbox" style={{ fontSize: 15, lineHeight: 1.65, color: '#B0B0B0', cursor: 'pointer' }}>
                      {smsCheckboxLabel}
                    </label>
                  </div>
              ...
              <div>
                ...
                {d.phone_sales && (
                  <div className="lp-reveal lp-d3" style={{ display: 'flex', gap: 18, alignItems: 'flex-start', marginBottom: 32 }}>
                    <div style={{ width: 48, height: 48, borderRadius: 12, background: `${c}0f`, border: `1px solid ${c}1a`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>📞</div>
                    <div>
                      <p style={{ fontSize: 11, color: '#333', textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: 4, fontWeight: 600 }}>{isGym || isCustomService ? 'Phone' : isInsurance ? 'Office' : 'Sales'}</p>
                      <p style={{ fontSize: 16, fontWeight: 500 }}>
      ...
      {page === 'privacy' && (
        <div style={{ padding: '140px 48px 100px', maxWidth: 720, margin: '0 auto', animation: 'fadeUp 0.5s cubic-bezier(0.16,1,0.3,1)' }}>
          ...
          <div style={{ fontSize: 14, color: '#666', lineHeight: 1.85, fontWeight: 300 }}>
            ...
            <p style={{ marginBottom: 16 }}>We do not sell, rent, or share mobile phone numbers, SMS opt-in data, or SMS consent status with third parties or affiliates for their marketing or promotional purposes. We will not share your SMS opt-in or consent status with any third party for purposes unrelated to providing the messaging services you requested. We may share personal data, including SMS opt-in or consent status, with service providers, phone companies, messaging platforms, and vendors only as needed to deliver and support those text messages.</p>
            ...
            <p style={{ marginBottom: 16 }}><strong style={{ color: '#E8E8E8', fontWeight: 500 }}>Privacy of mobile information:</strong> Mobile phone numbers, SMS opt-in data, and SMS consent status will not be sold, rented, or shared with third parties or affiliates for their marketing or promotional purposes. We may share this information with vendors that help provide messaging services only as needed to deliver and support the messages you requested.</p>
          ...
          <div style={{ background: '#161616', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 20, padding: '48px 44px 40px', maxWidth: 420, width: '100%', textAlign: 'center' as const, animation: 'modalIn 0.4s cubic-bezier(0.34,1.56,0.64,1)', boxShadow: '0 32px 100px rgba(0,0,0,0.6)', position: 'relative' as const }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, background: `${c}0d`, border: `1px solid ${c}1a`, borderRadius: 12, padding: '16px 18px', textAlign: 'left' as const, marginBottom: 24 }}>
              ...
              <div>
                ...
                <span style={{ fontSize: 12, color: '#666', lineHeight: 1.5 }}>If you opted in to SMS, you'll receive a text confirmation shortly with your appointment details.</span>
              </div>
            </div>
